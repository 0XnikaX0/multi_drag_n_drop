...
function render() {
    ...
    for (const elem of Object.values(state.elemek)){
        dobozHTML+=`
        <div class="box ${state.draggedId === elem.id ? "grabbed" : "not-grabbed"}" style="position: absolute; left: ${elem.x}px; top: ${elem.y}px;" onmousedown="dobozDragStart(window.event)" onmouseup="dobozDragStop(window.event)" onmousemove="dobozMouseMove(window.event)" data-egyedi-azonosito=${elem.id}>
            <div class="card-body">
